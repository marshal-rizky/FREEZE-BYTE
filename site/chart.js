 ...
 * overshoot di sekitar titik balik tajam -- dan di grafik harga itu berarti
 * menggambar harga yang tidak pernah terjadi. Interpolasi monotonik tidak
 * pernah melewati nilai data di antara dua titik, jadi kurvanya halus tanpa
 * mengarang angka. ALKA punya lompatan +97% dalam enam baris; persis bentuk
 * yang membuat spline naif meleset.
 */
...
function renderPriceChart(container, data, readout) {
  ...
  // plot. Label tidak boleh masuk ke dalam plot: di sanalah bandnya berada,
  // dan teks di atas band merah sulit dibaca.
  ...
  const defs = el("defs", {});
  defs.appendChild(gradient("priceFill", 0.26));
  svg.appendChild(defs);

  for (let i = 0; i <= 4; i += 1) {
    const value = yMin + ((yMax - yMin) * i) / 4;
    svg.appendChild(el("line", { x1: PAD.left, y1: y(value), x2: W - PAD.right, y2: y(value), class: "grid" }));
    svg.appendChild(el("text", { x: PAD.left - 10, y: y(value) + 4, class: "axis-label y" }, idFmt(Math.round(value))));
  }

  // Label jendela ditempatkan dalam dua tahap. Jendela yang berdekatan --
  // ALKA punya jendela 27 dan 29 Juli yang hampir bersentuhan -- akan
  // menabrakkan labelnya kalau semuanya dipasang di baris yang sama, dan
  // jendela di tepi kanan akan terpotong batas viewBox. Jadi setiap label
  // dijepit ke area plot lalu diturunkan satu baris sampai tidak bertabrakan.
  const CHAR_W = 5.2;   // perkiraan lebar karakter pada font-size 10px
  const LABEL_GAP = 6;
  const ROW_H = 12;
  const LABEL_TOP = PAD.top - 28;
  const placed = [];
  ...
  data.windows.forEach((win) => {
    ...
    svg.appendChild(el("rect", {
      x: left, y: PAD.top, width: width, height: plotH, class: `freeze-band ${kind}`,
    }));
    // Tepi ditegaskan supaya batas jendela terbaca persis, dan gaya garisnya
    // (padat vs putus) ikut membedakan statusnya.
    [left, left + width].forEach((edge) => {
      svg.appendChild(el("line", {
        x1: edge, y1: PAD.top, x2: edge, y2: PAD.top + plotH, class: `freeze-band ${kind}-edge`,
      }));
    });

    const text = win.confirmed ? "beku (terkonfirmasi)" : "volume nol (tersimpulkan)";
    const halfW = (text.length * CHAR_W) / 2;
    const cx = Math.min(Math.max(left + width / 2, PAD.left + halfW), W - PAD.right - halfW);
    const span = { from: cx - halfW - LABEL_GAP, to: cx + halfW + LABEL_GAP };

    let row = 0;
    while (placed.some((p) => p.row === row && p.from < span.to && span.from < p.to)) row += 1;
    placed.push({ row, from: span.from, to: span.to });

    svg.appendChild(el("text", { x: cx, y: LABEL_TOP + row * ROW_H, class: `freeze-label ${kind}` }, text));
  });

  rows.forEach((row, i) => {
    if (i % 5 !== 0) return;
    svg.appendChild(el("text", { x: x(i), y: H - PAD.bottom + 20, class: "axis-label x" }, row.date.slice(5)));
  });
  ...
  const cross = el("line", { x1: 0, y1: PAD.top, x2: 0, y2: PAD.top + plotH, class: "crosshair", opacity: "0" });
  ...
  const show = (i) => {
    ...
    if (readout) {
      readout.innerHTML =
        ...
        `<span>volume <strong>${row.volume === 0 ? "0 — tidak ada transaksi" : idFmt(row.volume)}</strong></span>`;
    }
  };
  const hide = () => {
  ...
  const hit = el("rect", { x: PAD.left, y: PAD.top, width: plotW, height: plotH, class: "hit" });
  hit.addEventListener("pointermove", (event) => {
    const box = svg.getBoundingClientRect();
    // Koordinat pointer dipetakan balik ke ruang viewBox; lebar SVG di layar
    // berubah mengikuti lebar panel.
    const vx = ((event.clientX - box.left) / box.width) * W;
    const ratio = (vx - PAD.left) / plotW;
    show(Math.max(0, Math.min(rows.length - 1, Math.round(ratio * (rows.length - 1)))));
  });
...
/* Sparkline untuk kartu bento: garis yang sama, tanpa sumbu dan tanpa label.
 * Jendela beku tetap diberi shading -- itu isi ceritanya, bukan hiasan. */
function renderSparkline(container, data) {
  ...
  const y = (v) => PAD + (H - PAD * 2) - ((v - yMin) / (yMax - yMin)) * (H - PAD * 2);
  const indexOf = (date) => rows.findIndex((r) => r.date === date);

  const svg = el("svg", {
    viewBox: `0 0 ${W} ${H}`, class: "spark", preserveAspectRatio: "none",
    role: "img", "aria-label": `Ringkasan pergerakan harga ${data.symbol}`,
  });
  const defs = el("defs", {});
  defs.appendChild(gradient("sparkFill", 0.3));
  svg.appendChild(defs);
  ...
  (data.windows || []).forEach((win) => {
    ...
    if (from < 0 || to < 0) return;
    svg.appendChild(el("rect", {
      x: x(from), y: 0, width: Math.max(x(to) - x(from), 2), height: H,
      class: `freeze-band ${win.confirmed ? "confirmed" : "inferred"}`,
    }));
  });
